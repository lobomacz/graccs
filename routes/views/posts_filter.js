...
exports = module.exports = function (req, res) {
	...
	locals.currentNewsFilter = filter;
		...
		switch (filter) {
			...
			case 'anual':
				q = keystone.list('Post').paginate({
					...
					.where('state', 'published')
					.where('publishedDate').gte(moment().startOf('year').toDate())
					.where('publishedDate').lte(moment().endOf('year').toDate())
					.sort('-publishedDate');
				break;
			case 'relevantes':
				q = keystone.list('Post').paginate({
