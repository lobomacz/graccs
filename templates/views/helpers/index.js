...
module.exports = function () {
	...
	_helpers.newsUrl = function (pageNumber, options) {
		return '/noticias?page=' + pageNumber;
	};

	_helpers.newsFilterUrl = function (pageNumber, filter, options) {
		return '/noticias-filtradas/' + filter + '?page=' + pageNumber;
	};

	_helpers.indicatorsFilterUrl = function (pageNumber, filter, options) {
		return '/indicadores/' + filter + '?page=' + pageNumber;
	};

	_helpers.indicatorsClassificationUrl = function (pageNumber, filter, classification, options) {
		return '/indicadores/' + filter + '/' + classification + '?page=' + pageNumber;
	};

	_helpers.searchUrl = function (pageNumber, query, options) {
		return '/buscar/?q=' + query + '&page=' + pageNumber;
	};
	...
	* */
	...
	_helpers.paginationNavigationNews = function (pages, currentPage, totalPages, options) {
		var html = '';
		...
		_.each(pages, function (page, ctr) {
			// create ref to page, so that '...' is displayed as text even though int value is required
			var pageText = '<span>' + page + '</span>';
			// create boolean flag state if currentPage
			var isActivePage = ((page === currentPage) ? true : false);
			// need an active class indicator
			var liClass = ((isActivePage) ? ' class="active pg-count"' : ' class="pg-count"');
			...
			// get the pageUrl using the integer value
			var pageUrl = _helpers.newsUrl(page);
			// wrapup the html
			html += '<li' + liClass + '>' + linkTemplate({ url: pageUrl, text: pageText }) + '</li>\n';
		});
	...
	_helpers.paginationNavigationNewsFilter = function (pages, currentPage, totalPages, filter, options) {
		var html = '';
		...
		_.each(pages, function (page, ctr) {
			...
			// get the pageUrl using the integer value
			var pageUrl = _helpers.newsFilterUrl(page, filter);
			// wrapup the html
			html += '<li' + liClass + '>' + linkTemplate({ url: pageUrl, text: pageText }) + '</li>\n';
		});
	...
	_helpers.paginationNavigationIndicatorsFilter = function (pages, currentPage, totalPages, filter, options) {
		var html = '';
		...
		_.each(pages, function (page, ctr) {
			...
			// get the pageUrl using the integer value
			var pageUrl = _helpers.indicatorsFilterUrl(page, filter);
			// wrapup the html
			html += '<li' + liClass + '>' + linkTemplate({ url: pageUrl, text: pageText }) + '</li>\n';
		});
	...
	_helpers.paginationNavigationIndicatorsClassification = function (pages, currentPage, totalPages, filter, classification, options) {
		var html = '';
		...
		_.each(pages, function (page, ctr) {
			...
			// get the pageUrl using the integer value
			var pageUrl = _helpers.indicatorsClassificationUrl(page, filter, classification);
			// wrapup the html
			html += '<li' + liClass + '>' + linkTemplate({ url: pageUrl, text: pageText }) + '</li>\n';
		});
	...
	_helpers.paginationNavigationSearch = function (pages, currentPage, totalPages, query, options) {
		var html = '';
		...
		_.each(pages, function (page, ctr) {
			...
			// get the pageUrl using the integer value
			var pageUrl = _helpers.searchUrl(page, query);
			// wrapup the html
			html += '<li' + liClass + '>' + linkTemplate({ url: pageUrl, text: pageText }) + '</li>\n';
		});
	...
	_helpers.paginationPreviousNewsUrl = function (previousPage) {
		if (previousPage === false) {
			previousPage = 1;
		}
		return _helpers.newsUrl(previousPage);
	};

	_helpers.paginationPreviousNewsFilterUrl = function (previousPage, filter) {
		if (previousPage === false) {
			previousPage = 1;
		}
		
		return _helpers.newsFilterUrl(previousPage, filter);
	};

	_helpers.paginationPreviousIndicatorsFilterUrl = function (previousPage, filter) {
		if (previousPage === false) {
			previousPage = 1;
		}

		return _helpers.indicatorsFilterUrl(previousPage, filter);
	};

	_helpers.paginationPreviousIndicatorsClassificationUrl = function (previousPage, filter, classification) {
		if (previousPage === false) {
			previousPage = 1;
		}

		return _helpers.indicatorsClassificationUrl(previousPage, filter, classification);
	};

	_helpers.paginationPreviousSearchUrl = function (previousPage, query) {
		if (previousPage === false) {
			previousPage = 1;
		}

		return _helpers.searchUrl(previousPage, query);
	};
	...
	_helpers.paginationNextNewsUrl = function (nextPage, totalPages) {
		if (nextPage === false) {
			nextPage = totalPages;
		}
		return _helpers.newsUrl(nextPage);
	};

	_helpers.paginationNextNewsFilterUrl = function (nextPage, totalPages, filter) {
		if (nextPage === false) {
			nextPage = totalPages;
		}
		
		return _helpers.newsFilterUrl(nextPage, filter);
	};

	_helpers.paginationNextIndicatorsFilterUrl = function (nextPage, totalPages, filter) {
		if (nextPage === false) {
			nextPage = totalPages;
		}

		return _helpers.indicatorsFilterUrl(nextPage, filter);
	};

	_helpers.paginationNextIndicatorsClassificationUrl = function (nextPage, totalPages, filter, classification) {
		if (nextPage === false) {
			nextPage = totalPages;
		}

		return _helpers.indicatorsClassificationUrl(nextPage, filter, classification);
	};

	_helpers.paginationNextSearchUrl = function (nextPage, totalPages, query) {
		if (nextPage === false) {
			nextPage = totalPages;
		}

		return _helpers.searchUrl(nextPage, query);
	};
